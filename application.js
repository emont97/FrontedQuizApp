// Seleccion del tema "dark" o el tema "light"
...
function questionScreen(type) {
    ...
    //recupera los datos del formulario segun la seleccion
    getQuiz(type);
}
...
// obteniendo promesas del cuestionario.
...
submit.addEventListener("click", function () {
    ...
    if (submit.textContent == "Ver Resultado") {
        showQuizComplete();
        return;
    }
    if (selectedBox = document.querySelector(".selected")) {

        // eliminar letra de la seleccion
    ...
    if (qCount >= (totalQuestions - 1)) {
        ...
        submit.textContent = "Siguiente Pregunta";
    }
    return;
})
